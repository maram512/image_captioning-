...
class ImageCaptioningModel(torch.nn.Module):
    ...
    def decode_greedy(self, images, max_len=30, start_idx=1, end_idx=2):
        ...
        with torch.no_grad():
            ...
            generated = torch.full((batch, 1), start_idx,
...
# --------------------------
# CACHED MODEL LOADING  (IMPORTANT)
# --------------------------
@st.cache_resource
def load_everything():
    # Load checkpoint from HF once only
    ...
    model = ImageCaptioningModel(vocab_size=vocab_size).to("cpu")
    model.load_state_dict(checkpoint["model_state"], strict=False)
    model.eval()

    return model, vocab

model, vocab = load_everything()
